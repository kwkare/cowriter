...
from cowriter.constants import nwFiles, nwQuotes, nwUnicode, trStats
...
class Config:
    ...
    cowriter.conf file between sessions. Most of the settings can be
    ...
    def __init__(self) -> None:
        ...
        self.appName   = "CoWriter"
        self.appHandle = "cowriter"
        # Set Paths
        ...
        self._aiSettings = None  # Initialized on first access
        self._lastAuthor = ""
    ...
    def loadConfig(self, splash: NSplashScreen | None = None) -> bool:
        ...
        if self._aiSettings:
            ...
            self._aiSettings.anthropic_api_key = conf.rdStr(sec, "anthropic_api_key", self._aiSettings.anthropic_api_key)
            self._aiSettings.anthropic_model = conf.rdStr(sec, "anthropic_model", self._aiSettings.anthropic_model)
            self._aiSettings.ollama_model = conf.rdStr(sec, "ollama_model", self._aiSettings.ollama_model)
            self._aiSettings.temperature = conf.rdFlt(sec, "temperature", self._aiSettings.temperature)
            self._aiSettings.max_tokens = conf.rdInt(sec, "max_tokens", self._aiSettings.max_tokens)
            self._aiSettings.enable_streaming = conf.rdBool(sec, "enable_streaming", self._aiSettings.enable_streaming)
            self._aiSettings.enable_autocomplete = conf.rdBool(sec, "enable_autocomplete", self._aiSettings.enable_autocomplete)
    ...
    def saveConfig(self) -> bool:
        ...
        if self._aiSettings:
            conf["AI"] = {
                "provider_type":      str(self._aiSettings.provider_type),
                "openai_api_key":     str(self._aiSettings.openai_api_key),
                "openai_model":       str(self._aiSettings.openai_model),
                "openai_base_url":    str(self._aiSettings.openai_base_url),
                "anthropic_api_key":  str(self._aiSettings.anthropic_api_key),
                "anthropic_model":    str(self._aiSettings.anthropic_model),
                "ollama_model":       str(self._aiSettings.ollama_model),
                "ollama_base_url":    str(self._aiSettings.ollama_base_url),
                "temperature":        str(self._aiSettings.temperature),
                "max_tokens":         str(self._aiSettings.max_tokens),
                "enable_streaming":   str(self._aiSettings.enable_streaming),
                "enable_autocomplete": str(self._aiSettings.enable_autocomplete),
            }
    ...
    @property
    def aiSettings(self) -> AISettings:
        """Return AI settings, initializing if needed."""
        if self._aiSettings is None:
            from cowriter.ai.settings import AISettings as _AISettings
            self._aiSettings = _AISettings()
        return self._aiSettings
