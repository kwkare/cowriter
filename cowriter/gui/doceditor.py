...
from cowriter.extensions.configlayout import NPathColorLabel
from cowriter.extensions.eventfilters import WheelEventFilter
from cowriter.extensions.modified import NIconToggleButton, NIconToolButton
from cowriter.gui.dochighlight import BLOCK_META, BLOCK_TITLE
from cowriter.gui.editordocument import GuiTextDocument
from cowriter.gui.theme import STYLES_MIN_TOOLBUTTON
from cowriter.text.counting import standardCounter
from cowriter.text.formats import processHeading
...
class GuiDocEditor(QPlainTextEdit):
    ...
    def __init__(self, parent: QWidget) -> None:
        ...
        # AI Context Menu Translations
        self._trAIContinue = self.tr("AI Continue Writing")
        self._trAIRewrite = self.tr("AI Rewrite Selection")
        self._trAIExpand = self.tr("AI Expand Selection")
        self._trAISummarize = self.tr("AI Summarize Selection")
        self._trAIChat = self.tr("AI Chat...")
    ...
    def aiContinue(self) -> None:
        ...
        text_before = cursor.selectedText().replace("\u2029", "\n")
        if text_before:
            from cowriter.gui.aichat import GuiAICompleteDialog
    ...
    def openAIChat(self) -> None:
        ...
        for child in self.children():
            if isinstance(child, GuiAIChatPanel):
                child.close()
                child.deleteLater()
                break
        dlg = GuiAIChatPanel(self)
        dlg.open()
