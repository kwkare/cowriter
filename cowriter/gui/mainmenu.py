...
from cowriter.common import openExternalPath, qtAddAction, qtAddMenu, qtLambda
from cowriter.constants import (
    nwConst, nwKeyWords, nwLabels, nwShortcode, nwStats, nwStyles, nwUnicode,
    trConst, trStats
)
from cowriter.enum import nwDocAction, nwDocInsert, nwFocus, nwView
from cowriter.extensions.eventfilters import StatusTipFilter
...
class GuiMainMenu(QMenuBar):
    ...
    def _buildAIMenu(self) -> None:
        """Assemble the AI menu."""
        self.aiMenu = qtAddMenu(self, self.tr("&AI"))

        # AI > Continue Writing
        self.aAIContinue = qtAddAction(self.aiMenu, self.tr("Continue Writing"))
        self.aAIContinue.setShortcut("Ctrl+Alt+C")
        self.aAIContinue.triggered.connect(self.mainGui.showAIContinue)

        # AI > Rewrite Selection
        self.aAIRewrite = qtAddAction(self.aiMenu, self.tr("Rewrite Selection"))
        self.aAIRewrite.setShortcut("Ctrl+Alt+R")
        self.aAIRewrite.triggered.connect(self.mainGui.showAIRewrite)

        # AI > Expand Selection
        self.aAIExpand = qtAddAction(self.aiMenu, self.tr("Expand Selection"))
        self.aAIExpand.setShortcut("Ctrl+Alt+E")
        self.aAIExpand.triggered.connect(self.mainGui.showAIExpand)

        # AI > Summarize Selection
        self.aAISummarize = qtAddAction(self.aiMenu, self.tr("Summarize Selection"))
        self.aAISummarize.setShortcut("Ctrl+Alt+S")
        self.aAISummarize.triggered.connect(self.mainGui.showAISummarize)

        self.aiMenu.addSeparator()

        # AI > AI Chat
        self.aAIChat = qtAddAction(self.aiMenu, self.tr("AI Chat..."))
        self.aAIChat.setShortcut("Ctrl+Alt+A")
        self.aAIChat.triggered.connect(self.mainGui.showAIChatPanel)
